...
def register_user(body):
    ...
    try:
        ...
        if existing_user:
            ...
            return {
                "statusCode": 400,
                "body": json.dumps({"error": "Username already registered."})
            }
...
def recalc_all_production():
    """
    Recalculate max_produce for all products in the production table.
    """
...
def create_stock(body):
    ...
    """
    try:
        # Now 'unit' is also required in the payload.
        ...
        # Recalculate max_produce for all products after a stock change.
...
def update_stock(body):
    ...
    try:
        ...
        # Use an alias for "unit" since it is a reserved keyword in DynamoDB.
...
def lambda_handler(event, context):
    ...
    4) Always return responses with CORS headers.
    """
    # Define CORS headers to add in every response.
    cors_headers = {
    ...
    try:
        ...
        if not http_method:
            # Support for Lambda Function URLs with payload version 2.0
        ...
        # Route the operation to the correct function
        ...
        # Ensure CORS headers are added to the response before returning.
