    ...
    for tx in transactions:
        ...
        if op in ["CreateStock", "AddStockQuantity", "SubtractDefectiveGoods"]:
            if op == "CreateStock":
                ...
                if "added_cost" in details:
                    added_cost = Decimal(str(details["added_cost"]))
                else:
                    added_cost = added_qty * Decimal(str(details.get("cost_per_unit", 0)))
                additions_qty += added_qty
                additions_amount += added_cost
...
def create_stock(body):
    """
    Creates a new stock item in the 'stock' table and offloads long tasks to a background thread.
    """
    try:
        required_fields = ["name", "quantity", "defective", "cost_per_unit", "stock_limit", "username", "unit"]
        ...
        total_cost = (available_quantity * cost_per_unit).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        stock_table = dynamodb.Table(stock_table_name)
        ...
        def background_tasks():
            try:
                ...
                log_transaction("CreateStock", transaction_details, username)
            except Exception as e:
...
# dynamodb, stock_table_name, logger, log_transaction, log_undo_action, recalc_all_production
...
def update_stock(body):
    ...
    try:
        ...
        new_total_cost = (available_quantity * new_cost_per_unit).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        ...
        # Save old state for undo logging
        old_state = {
        ...
        # Offload logging and production recalculation to a background thread
        def background_tasks():
            try:
                ...
                logger.error(f"Error logging transaction in update_stock: {e}")
            try:
                log_undo_action("UpdateStock", {"item_id": item_id, "old_state": old_state}, username)
            except Exception as e:
                logger.error(f"Error logging undo action in update_stock: {e}")
            try:
                recalc_all_production()
            except Exception as e:
                logger.error(f"Error recalculating production in update_stock: {e}")
        ...
        response_body = {
            ...
            "quantity": float(available_quantity),
            "defective": float(defective),
            "total_quantity": float(new_quantity),
...
def delete_stock(body):
    """
    Deletes a stock item from the 'stock' table.
    """
    try:
        ...
        def background_tasks():
            try:
                ...
                logger.error(f"Error logging transaction in delete_stock: {e}")
            try:
                log_undo_action("DeleteStock", {"deleted_item": deleted_item}, username)
            except Exception as e:
                logger.error(f"Error logging undo action in delete_stock: {e}")
            try:
                recalc_all_production()
            except Exception as e:
                logger.error(f"Error recalculating production in delete_stock: {e}")
...
def add_stock_quantity(body):
    """
    Adds quantity to an existing stock item.
    """
    try:
        ...
        added_cost = (cost_per_unit * quantity_to_add).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        new_total_cost = (Decimal(str(item.get("total_cost", 0))) + added_cost).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        
        stock_table.update_item(
            Key={"item_id": name},
            UpdateExpression="SET total_quantity = :t, quantity = :q, total_cost = :tc, updated_at = :updated_at",
            ExpressionAttributeValues={
                ":t": new_total,
                ":q": new_available,
                ":tc": new_total_cost,
        ...
        def background_tasks():
            try:
                ...
                logger.error(f"Error logging transaction in add_stock_quantity: {e}")
            try:
                log_undo_action("AddStockQuantity", {"item_id": name, "quantity_added": quantity_to_add}, username)
            except Exception as e:
                logger.error(f"Error logging undo action in add_stock_quantity: {e}")
            try:
                recalc_all_production()
            except Exception as e:
                logger.error(f"Error recalculating production in add_stock_quantity: {e}")
...
def subtract_stock_quantity(body):
    """
    Subtracts quantity from an existing stock item.
    """
    try:
        ...
        cost_per_unit = Decimal(str(item.get("cost_per_unit", 0)))
        sub_cost = (cost_per_unit * quantity_to_subtract).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        new_total_cost = (Decimal(str(item.get("total_cost", 0))) - sub_cost).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        if new_total_cost < 0:
        ...
        def background_tasks():
            try:
                ...
                logger.error(f"Error logging transaction in subtract_stock_quantity: {e}")
            try:
                log_undo_action("SubtractStockQuantity", {"item_id": name, "quantity_subtracted": quantity_to_subtract}, username)
            except Exception as e:
                logger.error(f"Error logging undo action in subtract_stock_quantity: {e}")
            try:
                recalc_all_production()
            except Exception as e:
                logger.error(f"Error recalculating production in subtract_stock_quantity: {e}")
...
def add_defective_goods(body):
    """
    Increments the 'defective' count of a stock item.
    """
    try:
        ...
        stock_table.update_item(
            Key={"item_id": name},
            UpdateExpression="SET defective = :d, quantity = :q, updated_at = :updated_at",
            ExpressionAttributeValues={
                ":d": new_defective,
                ":q": new_available,
                ":updated_at": now_str,
        ...
        def background_tasks():
            try:
                ...
                logger.error(f"Error logging transaction in add_defective_goods: {e}")
            try:
                log_undo_action("AddDefectiveGoods", {"item_id": name, "defective_added": defective_to_add}, username)
            except Exception as e:
                logger.error(f"Error logging undo action in add_defective_goods: {e}")
            try:
                recalc_all_production()
            except Exception as e:
                logger.error(f"Error recalculating production in add_defective_goods: {e}")
...
def subtract_defective_goods(body):
    """
    Decrements the 'defective' count of a stock item.
    """
    try:
        ...
        def background_tasks():
            try:
                ...
                logger.error(f"Error logging transaction in subtract_defective_goods: {e}")
            try:
                log_undo_action("SubtractDefectiveGoods", {"item_id": name, "defective_subtracted": defective_to_subtract}, username)
            except Exception as e:
                logger.error(f"Error logging undo action in subtract_defective_goods: {e}")
            try:
                recalc_all_production()
            except Exception as e:
                logger.error(f"Error recalculating production in subtract_defective_goods: {e}")
...
def convert_decimals(obj):
    """
    Recursively converts Decimal instances in a data structure to float.
    """
...
def get_all_stocks(body):
    """
    Returns all items from the 'stock' table.
    """
    try:
        ...
        # Convert any Decimal values to float so that JSON can serialize the items.
        items_converted = convert_decimals(items)
        
        logger.info(f"User '{username}' retrieved all stock items.")
        return {"statusCode": 200, "body": json.dumps(items_converted)}
...
# Assumed external definitions:
# dynamodb, production_table_name, stock_table_name, push_production_table_name,
# transactions_table_name, undo_table_name, logger, log_transaction, log_undo_action,
# recalc_max_produce, get_undo_record, mark_undo_as_done, get_current_stock_summary,
# extract_consumption_details, summarize_consumption_details, compute_consumption_amount,
# initialize_tables
...
def create_product(body):
    """
    Creates a new product in the 'production' table.
    If 'product_id' is not provided in the request, one is generated automatically.
    """
    try:
        # 'product_id' is optional now. Required fields for creation are:
        required = ["product_name", "stock_needed", "username"]
        for field in required:
            if field not in body:
                return {"statusCode": 400, "body": json.dumps({"error": f"'{field}' is required"})}
        
        # Use provided product_id if available; otherwise, generate one.
        ...
        production_table = dynamodb.Table(production_table_name)
        production_table.put_item(Item=product_item)
        threading.Thread(target=lambda: (
            log_transaction("CreateProduct", 
                            {"product_id": product_id, "product_name": product_name, "stock_needed": stock_needed}, 
                            username),
            log_undo_action("CreateProduct", {"product_id": product_id}, username)
        )).start()
        response = {"message": "Product created successfully.", "product_id": product_id}
        return {"statusCode": 200, "body": json.dumps(convert_decimals(response))}
    except Exception as e:
        logger.error(f"Error in create_product: {e}")
        return {"statusCode": 500, "body": json.dumps({"error": f"Internal error in create_product: {str(e)}"})}

def update_product(body):
    """
    Updates an existing product in the 'production' table.
    """
    try:
        ...
        if "stock_needed" in body:
            ...
            for item_id, qty_needed in new_stock_needed_converted.items():
                ...
                cost_for_item = (cost_per_unit * qty_needed).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
                cost_breakdown[item_id] = cost_for_item
                production_cost_total = (production_cost_total + cost_for_item).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
            if max_produce is None:
                max_produce = Decimal("0")
        else:
        ...
        # Offload logging and undo actions in background
        threading.Thread(target=lambda: (
        ...
        response = {
            ...
            "max_produce": int(max_produce),
            "production_cost_breakdown": {k: float(Decimal(v).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP))
                                            for k, v in {k: str(v) for k, v in cost_breakdown.items()}.items()},
...
def delete_product(body):
    """
    Deletes a product from the 'production' table.
    """
    try:
        required = ["product_id", "username"]
        for field in required:
...
def push_to_production(body):
    """
    Deducts stock items to produce a certain quantity of a product.
    """
    try:
        ...
        cost_per_unit_total = cost_per_unit_total.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        total_production_cost = (cost_per_unit_total * quantity_to_produce).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        ...
        threading.Thread(target=lambda: (
            log_transaction("PushToProduction", {"push_id": push_id, "product_id": product_id, 
                                                  "quantity_produced": quantity_to_produce, 
                                                  "deductions": required_deductions, 
                                                  "production_cost_per_unit": cost_per_unit_total, 
                                                  "total_production_cost": total_production_cost}, username),
...
def undo_production(body):
    """
    Undo a push_to_production by restoring the deducted stock.
    """
    try:
        ...
        response = {"message": f"Push '{push_id}' undone successfully."}
        return {"statusCode": 200, "body": json.dumps(convert_decimals(response))}
    except Exception as e:
...
def get_all_push_to_production(body):
    """
    Returns all items from the 'push_to_production' table.
    """
    try:
        username = body.get("username", "Unknown")
        push_table = dynamodb.Table(push_production_table_name)
        ...
        return {"statusCode": 200, "body": json.dumps(convert_decimals(items))}
    except Exception as e:
...
def delete_push_to_production(body):
    """
    Deletes a record from the 'push_to_production' table.
    """
    try:
        required = ["push_id", "username"]
        for field in required:
        ...
        response = {"message": f"Push record '{push_id}' deleted successfully."}
        return {"statusCode": 200, "body": json.dumps(convert_decimals(response))}
    except Exception as e:
...
def undo_action(body):
    """
    Generic "UndoAction" operation that looks up the user's active undo record
    and reverts the corresponding operation if possible.
    """
    try:
        ...
        if "undo_id" in body and body["undo_id"]:
            ...
            table = dynamodb.Table(undo_table_name)
            response = table.scan(FilterExpression=Attr("username").eq(username) & Attr("status").eq("ACTIVE"))
            active_records = response.get("Items", [])
            if not active_records:
                return {"statusCode": 404, 
                        "body": json.dumps({"error": "No active undo records found for the user."})}
        ...
        if not record:
            return {"statusCode": 404, 
                    "body": json.dumps({"error": "Undo record not found."})}
        if record["status"] != "ACTIVE":
            return {"statusCode": 400, 
                    "body": json.dumps({"error": "This undo record is already undone."})}
        ...
        if operation == "CreateStock":
            ...
            old_state = details["old_state"]
            update_body = {"product_id": details["product_id"], "product_name": old_state["product_name"], "stock_needed": old_state["stock_needed"], "username": username}
...
# =============================================================================
# REPORTS (DAILY/WEEKLY/MONTHLY) WITH CONSUMPTION SUMMARY
# =============================================================================
...
def get_existing_stock_record(operation, report_date):
    try:
        ...
        transactions_table = dynamodb.Table(transactions_table_name)
        resp = transactions_table.scan(FilterExpression=Attr("operation_type").eq(operation) & Attr("date").eq(report_date))
...
def save_opening_stock(body):
    """
    Saves or updates the 'opening stock' record for the current date.
    """
    try:
        ...
        details = {"opening_stock_qty": current_qty, "opening_stock_amount": current_amount}
        transactions_table = dynamodb.Table(transactions_table_name)
        ...
        if existing:
            transactions_table.update_item(
                ...
                ExpressionAttributeValues={":d": details, ":t": timestamp_str, ":r": report_date},
            )
...
def save_closing_stock(body):
    """
    Saves or updates the 'closing stock' record for the current date,
    also calculates consumption based on the difference from opening stock.
    """
    try:
        if "username" not in body:
            return {"statusCode": 400, "body": json.dumps({"error": "'username' is required"})}
        ...
        tx_resp = transactions_table.scan(FilterExpression=Attr("timestamp").between(opening_record["timestamp"], closing_timestamp))
        ...
        details = {"closing_stock_qty": closing_qty, "closing_stock_amount": closing_amount, "consumption_qty": consumption_qty, "consumption_amount": consumption_amount}
        existing = get_existing_stock_record("SaveClosingStock", report_date)
        if existing:
            transactions_table.update_item(
                Key={"transaction_id": existing["transaction_id"]},
                UpdateExpression="SET details = :d, #ts = :t, #dt = :r, username = :u",
                ExpressionAttributeNames={"#ts": "timestamp", "#dt": "date"},
                ExpressionAttributeValues={":d": details, ":t": closing_timestamp, ":r": report_date, ":u": username},
            )
        ...
        response = {
            "message": response_message,
            "closing_stock_qty": float(closing_qty),
...
def get_all_stock_transactions(body):
    """
    Returns all items from the 'stock_transactions' table, sorted by timestamp.
    """
    try:
        transactions_table = dynamodb.Table(transactions_table_name)
        items = []
...
def get_daily_consumption_summary(body):
    ...
    try:
        ...
        transactions_table = dynamodb.Table(transactions_table_name)
        tx_resp = transactions_table.scan(FilterExpression=Attr("date").eq(report_date))
        transactions = tx_resp.get("Items", [])
        while "LastEvaluatedKey" in tx_resp:
            tx_resp = transactions_table.scan(FilterExpression=Attr("date").eq(report_date), ExclusiveStartKey=tx_resp["LastEvaluatedKey"])
            transactions.extend(tx_resp.get("Items", []))
        consumption_details = extract_consumption_details(transactions)
...
def get_weekly_consumption_summary(body):
    ...
    try:
        ...
        transactions_table = dynamodb.Table(transactions_table_name)
        tx_resp = transactions_table.scan(FilterExpression=Attr("date").between(start_date, end_date))
        transactions = tx_resp.get("Items", [])
        while "LastEvaluatedKey" in tx_resp:
            tx_resp = transactions_table.scan(FilterExpression=Attr("date").between(start_date, end_date), ExclusiveStartKey=tx_resp["LastEvaluatedKey"])
            transactions.extend(tx_resp.get("Items", []))
        consumption_details = extract_consumption_details(transactions)
        consumption_summary = summarize_consumption_details(consumption_details)
        consumption_amount = compute_consumption_amount(transactions)
...
def get_monthly_consumption_summary(body):
    ...
    try:
        ...
        transactions_table = dynamodb.Table(transactions_table_name)
        tx_resp = transactions_table.scan(FilterExpression=Attr("date").between(start_date_str, end_date_str))
        transactions = tx_resp.get("Items", [])
        while "LastEvaluatedKey" in tx_resp:
            tx_resp = transactions_table.scan(FilterExpression=Attr("date").between(start_date_str, end_date_str), ExclusiveStartKey=tx_resp["LastEvaluatedKey"])
            transactions.extend(tx_resp.get("Items", []))
        consumption_details = extract_consumption_details(transactions)
        consumption_summary = summarize_consumption_details(consumption_details)
        consumption_amount = compute_consumption_amount(transactions)
...
def lambda_handler(event, context):
    try:
        ...
        if not operation:
            return add_cors_headers({"statusCode": 400, "body": json.dumps({"error": "Missing 'operation' field"})})
        ...
        logger.info("Operation response: " + json.dumps(response))
        return add_cors_headers(response)
    except json.JSONDecodeError:
        logger.error("JSON decode error for event: " + json.dumps(event))
        return add_cors_headers({"statusCode": 400, "body": json.dumps({"error": "Invalid JSON format"})})
    except Exception as e:
        logger.error(f"Error in lambda_handler: {e}")
        return add_cors_headers({"statusCode": 500, 
                                 "body": json.dumps({"error": f"Internal error: {str(e)}"})})
